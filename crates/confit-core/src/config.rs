...
/// Resolve a single provider URI. Returns `(resolved_value, is_secret)`.
...
pub fn resolve_providers(
    ...
    resolve_providers_inner(node, providers, runtime_vars, cwd, "", secrets)
}
...
fn resolve_providers_inner(
    ...
    match node {
        Value::String(s) => {
            let (resolved, is_secret) = resolve_provider(s, providers, runtime_vars, cwd)?;
        ...
        Value::Table(map) => {
            ...
            for (k, v) in map {
                ...
                new_map.insert(
                    k.clone(),
                    resolve_providers_inner(v, providers, runtime_vars, cwd, &path, secrets)?,
                );
            }
            Ok(Value::Table(new_map))
        }
        ...
        match node {
            ...
            Value::String(s) => {
                ...
                match interpolate_value(s, config, &resolving)
                    ...
                    .and_then(|v| {
                        resolve_provider(&v, providers, merged_vars, Some(config_dir))
                            .map(|(val, _)| val)
                    }) {
...
mod tests {
    ...
    fn test_file_provider() {
        ...
        let (result, secret) = resolve_provider(&uri, &providers, &vars, None).unwrap();
    ...
    fn test_file_provider_relative() {
        ...
        let vars = HashMap::new();
        let (result, _) =
            resolve_provider("file://token.txt", &providers, &vars, Some(dir.path())).unwrap();
    ...
    fn test_file_provider_missing() {
        let providers = Value::Table(Map::new());
        let vars = HashMap::new();
        let result = resolve_provider("file:///nonexistent/file.txt", &providers, &vars, None);
        assert!(result.is_err());
    ...
    fn test_custom_provider() {
        ...
        .unwrap();
        let vars = HashMap::new();
        let (result, _) = resolve_provider("echo://some/path", &providers, &vars, None).unwrap();
    ...
    fn test_custom_provider_with_vars() {
        ...
        let (result, _) = resolve_provider("vault://db/password", &providers, &vars, None).unwrap();
    ...
    fn test_secret_scheme_with_literal() {
        ...
        let vars = HashMap::new();
        let (result, secret) =
            resolve_provider("secret://literal-value", &providers, &vars, None).unwrap();
    ...
    fn test_secret_scheme_with_custom_provider() {
        ...
        let vars = HashMap::new();
        let (result, secret) =
            resolve_provider("secret://echo://data", &providers, &vars, None).unwrap();
    ...
    fn test_non_secret_not_flagged() {
        ...
        .unwrap();
        let vars = HashMap::new();
        let (_, secret) = resolve_provider("echo://whatever", &providers, &vars, None).unwrap();
        assert!(!secret);
    }
    ...
    fn test_secret_tracking_in_resolve_providers() {
        ...
        let mut secrets = HashSet::new();
        let result =
            resolve_providers(&node, &providers, &vars, Some(dir.path()), &mut secrets).unwrap();
        let table = result.as_table().unwrap();
